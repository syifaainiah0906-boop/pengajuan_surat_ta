...
<div class="min-h-screen bg-gray-100 py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-gray-200 rounded-lg p-6 mb-6">
            <form method="GET" action="{{ route('admin.verifikasi.penelitian.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-gray-700">
                    @forelse($data_penelitian as $index => $item)
                    <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                        <td class="py-4 px-6">{{ $index + 1 + ($data_penelitian->currentPage() - 1) * $data_penelitian->perPage() }}</td>
                        ...
                        <td class="py-4 px-6 text-center space-x-2 flex justify-center items-center">
                            <a href="{{ route('admin.verifikasi.penelitian.show', $item->id) }}" class="text-blue-600 hover:text-blue-900 font-bold text-sm">Detail</a>

                            @if($item->status == 'pending')
                                <span class="text-gray-300">|</span>
                                
                                <form action="{{ route('admin.verifikasi.penelitian.update', $item->id) }}" method="POST" onsubmit="return confirm('Setujui pengajuan ini?')">
                                    @csrf @method('PATCH')
                                    <input type="hidden" name="status" value="disetujui">
                                    <button type="submit" class="text-green-600 hover:text-green-900 font-bold text-sm" title="Setujui">&#10003;</button>
                                </form>

                                <form action="{{ route('admin.verifikasi.penelitian.update', $item->id) }}" method="POST" onsubmit="return confirm('Tolak pengajuan ini?')">
                                    @csrf @method('PATCH')
