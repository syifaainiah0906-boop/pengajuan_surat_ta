...
@section('title', 'Data Total Disetujui')

@section('content')
<div class="min-h-screen bg-gray-100 py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <h1 class="text-3xl font-bold text-gray-700 mb-8">Total Disetujui</h1>

        <div class="bg-gray-200 rounded-lg p-6 mb-6">
            <form method="GET" action="{{ route('admin.verifikasi.disetujui.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
        ...
        <div class="bg-white rounded-lg shadow-lg overflow-hidden">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-gray-700">
                    @forelse($data_disetujui as $index => $item)
                    <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                        <td class="py-4 px-6">{{ $index + 1 + ($data_disetujui->currentPage() - 1) * $data_disetujui->perPage() }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="py-8 text-center text-gray-500 italic">Belum ada data yang disetujui.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
